...
class QRMask{
	...
	public function makeMaskNo($maskNo, $width, $s, &$d, $maskGenOnly = false){
		...
		if(SFQRCode::QR_CACHEABLE){
			if(file_exists($fileName)){
				...
				if(!file_exists(SFQRCode::getInstance()->getDataFolder() . 'mask_' . $maskNo))
					mkdir(SFQRCode::getInstance()->getDataFolder() . 'mask_' . $maskNo);
	...
	public function calcN1N3($length){
		...
		for($i = 0; $i < $length; $i++){
			...
			if($this->runLength[$i] >= 5){
				$demerit += (SFQRCode::N1 + ($this->runLength[$i] - 5));
			}
			if($i & 1){
	...
	public function mask($width, $frame, $level){
		...
		if(SFQRCode::QR_FIND_FROM_RANDOM !== false){

			$howManuOut = 8 - (SFQRCode::QR_FIND_FROM_RANDOM % 9);
		...
		foreach($checked_masks as $i){
			...
			$demerit = (int) ((int) (abs($blacks - 50) / 5) * SFQRCode::N4);
